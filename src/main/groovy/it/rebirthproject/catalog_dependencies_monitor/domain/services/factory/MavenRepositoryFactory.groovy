...
import it.rebirthproject.catalog_dependencies_monitor.domain.constants.Constants
...
class MavenRepositoryFactory {
    ...
    static MavenRepository create(
            ...
            JsonSlurper jsonReader,
            List<String> libVersionFilters) {

        final MavenRepositoryVersion mavenVersion = MavenRepositoryVersion.fromStringIgnoreCase(requestedVersion).orElse(Constants.DEFAULT_MAVEN_CENTRAL_REPOSITORY_VERSION)

        return switch (mavenVersion) {
            case MavenRepositoryVersion.V1 -> new MavenV1Repository(httpClient, new MavenV1DependencyResponseMapper(versionComparator, jsonReader, libVersionFilters))
            case MavenRepositoryVersion.V2 -> new MavenV2Repository(httpClient, new MavenV2DependencyResponseMapper(versionComparator, libVersionFilters))
        }
    }
}
